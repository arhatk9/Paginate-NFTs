...
const Home = () => {
  ...
  const fetchNFTs = async () => {
    setLoading(true);
    const myNfts = await mx.nfts().findAllByOwner( new PublicKey(address));
    setNftList(myNfts);
    setCurrentPage(1);
    setLoading(false);
  };
  ...
  useEffect(() => {
    ...
    const exec = async () => {
      const stind = (currentPage - 1) * perPage;
      const endind = currentPage * perPage;
      await loadData(stind, endind);
      setCurrentView(nftList.slice(stind, endind));
      setLoading(false);
    };
    exec();
  }, [nftList, currentPage]);
  ...
  const loadData = async (startIndex, endIndex) => {
    const nftload = nftList.filter((nft, index) => {
      return (index >= startIndex && index < endIndex && nft.metadataTask.isPending());
    });
    const promises = nftload.map((nft) => nft.metadataTask.run());
